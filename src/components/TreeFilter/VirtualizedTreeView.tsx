...
export class VirtualizedTreeView extends React.PureComponent<IVirtualizedTreeViewProps, IVirtualizedTreeViewState> {
    ...
    private _renderItemValueWithTooltip(treeItem: TreeItem) {
        return (
            <Tooltip
                {...treeItem.itemTooltipInfo}
                directionalHint={DirectionalHint.rightCenter}
                delayMs={500}
            >
                <span>{treeItem.value}</span>
            </Tooltip>
        );
    }

    private _renderItemValue(treeItem: TreeItem) {
        return (
            <span>{treeItem.value}</span>
        );
    }
            ...
            if (this.props.isSingleSelect) {
                ...
                const SingleSelectItem = ({ }) =>
                    ...
                    >
                        ...
                        {treeItem.itemTooltipInfo ? this._renderItemValueWithTooltip(treeItem) : this._renderItemValue(treeItem)}
                    </span>;
